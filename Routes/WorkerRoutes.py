...
def add_job():
    try:
        ...
        last_user_package = Services.Package.get_last_user_package(user["id"])
        if last_user_package:
            ...
            product_quantity = int(last_user_package["product_quantity"]) + 1
            ...
            package = Services.Package.create(user["id"], 1, product["price"])
            # Add package detail
